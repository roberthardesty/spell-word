...
**Suggested next slice**: `#39` (TFLM arena precursor) is the only remaining AFK code slice, but its acceptance criterion ("invoke time in 30–40 ms range") is hardware-bound — ship the code change and file a follow-up for bench verification. Alternatives both need hardware: `#25` (UI state-machine IDF wrapper, HITL) or `#30` (W-recovery integration — needs `#39` first for the latency budget). With #24 landed, every pure-AFK tooling slice in Phase 5 is now done; the remaining work either needs a board or depends on #39.
