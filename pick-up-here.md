...
- **Phase 5** (decoder + W-recovery), remaining work:
  ...
  - **#52 follow-up: host round-trip test for `feat_extract`** (deferred from #52's AC item 4): the AFK code change for #52 landed 2026-05-08, but the formally-listed "host round-trip test (Python pipeline ↔ firmware `feat_extract`, top-1 letter index match)" was deferred. `feat_extract.c` depends on esp-dsp's `dsps_fft2r_*` (Xtensa SIMD), so a host build needs either a host FFT replacement (~50 lines of Cooley-Tukey) or a Python reference of the same MFCC pipeline that the firmware emits. The latter is more useful long-term — it doubles as documentation of the byte-exact frontend math, and once the firmware can dump a feature tensor over serial it becomes a one-shot bench check. On-device HITL accuracy (#52's AC item 5) is the more direct verification and should land first. Estimated effort if pursued: ~1 hour for a numpy reference + a bundled-WAV unit test; up to a day if the host FFT port is preferred for a true byte-for-byte match.
