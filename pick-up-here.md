...
| **#24** | `decoder_replay` host CLI + fixture format | AFK | Newly unblocked by #21. CSV fixture replay sharing `decoder_core` with the unit tests; all `spell_config.h` tunables overridable by CLI flag. Pairs with #22 — same `decoder_core` linkage. Mirror the `tools/w_detector_replay/main.c` pattern. |
...
**Suggested next slice**: `#22` (decoder IDF wrapper + partition loaders). Tightest continuation of the decoder critical path — `decoder_core`'s full-EOW resolver, early-commit predicate, and the unified `LETTER_RECOGNIZED` event are all in place; this just wires them to `esp_event` and the model/dictionary partitions. Unblocks `#30` (W-recovery integration). Alternatives: `#24` (decoder host CLI — pure host build, mirror the freshly-landed `tools/w_detector_replay/main.c` pattern) or `#27` (PCM ring — also feeds #30).
